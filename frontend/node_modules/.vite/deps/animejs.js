...
var isValidSVGAttribute = (el, propertyName) => {
  if (svgCssReservedProperties.includes(propertyName))
    return false;
      ...
      for (let p in params) {
        if (isKey(p)) {
          ...
          for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
            ...
            if (!isNil(inlineValue))
              inlineStylesStore[propName] = null;
            const tween = {
              parent: this,
              id: tweenId++,
              property: propName,
...
var Draggable = class {
  ...
  refresh() {
    ...
    if (isNum(parsedDragThreshold)) {
      ...
      if (!isUnd(mouse))
        dragThreshold.mouse = mouse;
      if (!isUnd(touch))
        dragThreshold.touch = touch;
    }
    this.containerArray = isArr(container) ? container : null;
    this.$container = /** @type {HTMLElement} */
    container && !this.containerArray ? parseTargets(
  ...
  handleMove(e) {
    ...
    if (canTouchScroll && (!isAtTop && !isAtBottom || isAtTop && movedY < 0 || isAtBottom && movedY > 0)) {
      ...
      preventDefault(e);
      if (!this.triggerStyles)
        this.triggerStyles = set(this.$trigger, { pointerEvents: "none" });
...
var processHTMLTemplate = (htmlTemplate, store, node, $parentFragment, type, debug, lineIndex, wordIndex, charIndex) => {
  ...
  if (debug)
    $highestParent.style.outline = `1px dotted ${debugColors2[type]}`;
...
var WAAPIAnimation = class {
  ...
  set currentTime(time) {
    ...
    this.forEach((anim) => {
      if (!this.persist && t >= this.duration)
        anim.play();
      anim.currentTime = t;
    });
  }
