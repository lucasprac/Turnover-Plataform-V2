...
Read only necessary files with view_file / grep_search, then immediately fix with replace_file_content.
Report results in 1-2 sentences (no checklists or detailed templates).
...
Execution Flow:
Present a checklist of about 3-7 main subtasks.
Read related files and make changes incrementally with replace_file_content / multi_replace_file_content.
Check for lint errors if possible (e.g., run lint command in terminal).
Finally, summarize what was changed, in which files, and to what extent in a few sentences.
...
Summarize task content in one line.
Check 1-2 relevant files with view_file / grep_search.
Immediately fix with replace_file_content.
Minimal verification as needed (e.g., visual check for type errors).
Report results in 1-2 sentences.
3.2 Standard Tasks (🟡)
Organize purpose, constraints, and expected impact scope in 2-3 sentences.
Present a checklist of about 3-7 items.
Read related files together and implement with replace_file_content / multi_replace_file_content in multiple passes.
Check for lint errors and fix immediately fixable ones on the spot.
Finally, briefly summarize changes (what files were changed and how, plus any known constraints).
3.3 Critical Tasks (🔴)
